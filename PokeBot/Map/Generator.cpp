...
void Generator::SortPokemon(std::map<int, std::vector<Pokemon_Data>> pokedex)
{
	for (int type = 1; type != (int)Type::Type_Max; type++)
	{
		...
		SortedPokemon.insert(std::pair<Type, std::map<int, std::vector<Pokemon_Data>>>((Type)type, std::map<int, std::vector<Pokemon_Data>>())
		);
...
Pokemon_Data Generator::RandomPokemon(Type type, int stage)
{
	...
	rng.seed(seedval);
	std::uniform_int_distribution<rng_type::result_type> udist(0, pool.size() - 1);
	rng_type::result_type random_number = udist(rng);

	return (pool[random_number]);
}